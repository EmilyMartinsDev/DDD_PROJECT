...
const customer_model_1 = require("../sequelize/customer.model");
class CustomerRepository {
    async create(entity) {
        await customer_model_1.CustomerModel.create({
            id: entity.id,
            name: entity.name,
    ...
    async update(entity) {
        await customer_model_1.CustomerModel.update({
            name: entity.name,
            street: entity.Address.street,
            number: entity.Address.number,
            zipcode: entity.Address.zip,
    ...
    async find(id) {
        let customerModel;
        try {
            customerModel = await customer_model_1.CustomerModel.findOne({
                where: {
                    id,
                },
                rejectOnEmpty: true,
    ...
    async findAll() {
        const customerModels = await customer_model_1.CustomerModel.findAll();
